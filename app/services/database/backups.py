...
def _get_pg_dump_env() -> Dict[str, str]:
    """从环境变量构建 pg_dump 所需的环境"""
...
async def create_backup_native(name: str, backup_dir: str, collections: Optional[List[str]] = None, user_id: str | None = None) -> Dict[str, Any]:
    """
    使用 PostgreSQL 原生 pg_dump 命令创建备份（推荐，速度快）
    """
    if not _check_pg_dump_available():
        raise Exception("pg_dump 命令不可用，请安装 PostgreSQL 客户端工具或使用 create_backup() 方法")

    db = get_mongo_db()

    backup_id = str(uuid.uuid4()).replace("-", "")[:24]
    timestamp = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
    backup_filename = f"backup_{name}_{timestamp}.sql.gz"
    ...
    cmd = ["pg_dump", "-h", host, "-p", port, "-U", user, "-d", database, "-F", "c", "-f", backup_path.replace(".gz", ".dump")]
    pg_env = _get_pg_dump_env()

    logger.info(f"🔄 开始执行 pg_dump 备份: {name}")

    def _run_pg_dump():
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=3600, env=pg_env)
        if result.returncode != 0:
            raise Exception(f"pg_dump 执行失败: {result.stderr}")
        # 压缩
        if os.path.exists(backup_path.replace(".gz", ".dump")):
            with open(backup_path.replace(".gz", ".dump"), "rb") as f_in:
                with gzip.open(backup_path, "wb") as f_out:
                    shutil.copyfileobj(f_in, f_out)
            os.remove(backup_path.replace(".gz", ".dump"))
        return result
    ...
    try:
        ...
        for p in [backup_path, backup_path.replace(".gz", ".dump")]:
            if os.path.exists(p):
                await asyncio.to_thread(os.remove if os.path.isfile(p) else shutil.rmtree, p)
        raise
    ...
    db.database_backups.insert_one(backup_meta)
