...
def _get_default_provider_by_model(model_name: str) -> str:
    ...
    model_provider_map = {
        ...
        'Pro/deepseek-ai/DeepSeek-V3.2-Exp': 'siliconflow',
        'stepfun-ai/Step-3.5-Flash': 'siliconflow',
    }

    provider = model_provider_map.get(model_name, 'siliconflow')  # 默认使用硅基流动
...
def create_analysis_config(
    ...
    # TA_RATE_SAFE_MODE=1 时跳过辩论阶段（看涨/看跌研究员），避免 429 卡住
    import os
    if os.getenv("TA_RATE_SAFE_MODE", "1").strip().lower() in {"1", "true", "yes", "on"}:
        config["max_debate_rounds"] = 0
        config["max_risk_discuss_rounds"] = 0
        logger.info("🔧 [限流保护] TA_RATE_SAFE_MODE=1，跳过辩论与风险讨论阶段")
...
class SimpleAnalysisService:
    ...
    def __init__(self):
        ...
        # 降低并发为 1，减少同时 LLM 调用，降低 429 限流概率
    ...
    async def _update_progress_async(self, task_id: str, progress: int, message: str):
        ...
        try:
            ...
            # 更新 PostgreSQL（get_mongo_db 返回同步客户端）
            from app.core.database import get_mongo_db_sync
            from datetime import datetime
            db = get_mongo_db_sync()
    ...
    async def create_analysis_task(
        ...
        try:
            ...
            try:
                ...
                existing = db.analysis_tasks.find_one({"task_id": task_id})
                if not existing:
                    db.analysis_tasks.insert_one(doc)
                logger.info(f"✅ 任务已保存到PostgreSQL: {task_id}")
    ...
    async def execute_analysis_background(
        ...
        try:
            ...
            # 中文分析师名 → 英文 key，否则 tracker 会生成 "市场分析师分析师" 等重复
            _cn_to_en = {"市场分析师": "market", "基本面分析师": "fundamentals", "新闻分析师": "news", "社交分析师": "social", "社交媒体分析师": "social"}
            _raw = request.parameters.selected_analysts if request.parameters else ["market", "fundamentals"]
            _analysts_for_tracker = [_cn_to_en.get(a, a) for a in _raw]
            ...
            def create_progress_tracker():
                ...
                tracker = RedisProgressTracker(
                    task_id=task_id,
                    analysts=_analysts_for_tracker,
            ...
            # 执行实际的分析（总超时保护，覆盖前置准备+图执行）
            total_timeout_seconds = int(os.getenv("TA_ANALYSIS_TOTAL_TIMEOUT_SECONDS", "900"))
            try:
                result = await asyncio.wait_for(
                    self._execute_analysis_sync(task_id, user_id, request, progress_tracker),
                    timeout=total_timeout_seconds
                )
            except asyncio.TimeoutError as timeout_err:
                logger.error(
                    f"❌ 分析总执行超时: task_id={task_id}, symbol={stock_code}, timeout={total_timeout_seconds}s"
                )
                raise RuntimeError(
                    f"分析总执行超时（>{total_timeout_seconds}秒）。请稍后重试，或降低研究深度/分析师数量。"
                ) from timeout_err
            ...
            while getattr(root, "__cause__", None) is not None:
                root = root.__cause__
            raw_error = f"{type(root).__name__}: {root}"
            ...
            # 使用原始异常进行格式化
            formatted_error = ErrorFormatter.format_error(raw_error, error_context)

            # 构建用户友好的错误消息（技术细节优先使用原始异常）
            tech = formatted_error.get("technical_detail", raw_error)
            # 若 root 与 e 不同，说明存在链式异常，raw_error 已是原始错误
            tech_display = raw_error
            if len(tech_display) > 500:
                tech_display = tech_display[:500] + "..."
            user_friendly_error = (
    ...
    def _run_analysis_sync(
        ...
        """同步执行分析的具体实现"""
        try:
            # 在线程启动时强制加载 .env（解决后端全流程取不到基本面数据的问题）
            # 必须保证 D:\soft\Stock_analysis\.env 最后加载以覆盖
            import os
            from pathlib import Path
            try:
                from dotenv import load_dotenv
                _ta_root = Path(__file__).resolve().parent.parent.parent  # TradingAgents
                _soft_root = _ta_root.parent  # d:/soft
                _stock_env = _soft_root / "Stock_analysis" / ".env"
                for p in [_soft_root / ".env", _ta_root / ".env", _stock_env]:
                    if p.exists():
            ...
            logger.info(f"[分析线程诊断] task_id={task_id} symbol={request.stock_code} FINNHUB={'已加载' if (fh and len(fh) > 8) else '未配置'} ALPHA_VANTAGE={'已加载' if (av and len(av) > 8) else '未配置'}")
            ...
            # 🔥 本地模型优先：如果系统配置使用本地 Ollama，覆盖前端传来的模型名
            import json
            settings_path = Path(__file__).parent.parent.parent / "config" / "settings.json"
            if settings_path.exists():
                with open(settings_path) as f:
                    sys_settings = json.load(f)
                if sys_settings.get("llm_provider") == "ollama":
                    quick_model = sys_settings.get("quick_think_llm", "qwen3-14b-reasoning")
                    deep_model = sys_settings.get("deep_think_llm", "qwen3-14b-reasoning")
                    logger.info(f"🏠 [本地模型优先] 覆盖为本地 Ollama 模型: {quick_model}")
            ...
            # 中文分析师名 -> 英文 analyst_type 映射（图结构需要英文 key）
            _analyst_cn_to_en = {
                "市场分析师": "market",
                "基本面分析师": "fundamentals",
                "新闻分析师": "news",
            ...
            selected_analysts = [_analyst_cn_to_en.get(a, a) for a in raw_analysts]

            # 429 重试次数：从环境变量读取，默认 5（高于默认 3 以应对限流）
            retry_times = int(os.getenv("TA_LLM_RETRY_TIMES", "5"))
            quick_model_config = {"retry_times": retry_times}
            deep_model_config = {"retry_times": retry_times}
            thread_logger.info(f"🔧 [429 重试] retry_times={retry_times} (来自 TA_LLM_RETRY_TIMES)")
            ...
            # 执行实际分析，增加硬超时保护，避免任务无限卡住
            analysis_timeout_seconds = int(os.getenv("TA_ANALYSIS_TIMEOUT_SECONDS", "900"))
            logger.info(f"⏱️ 分析超时保护已启用: {analysis_timeout_seconds}s")

            from concurrent.futures import ThreadPoolExecutor, TimeoutError as FutureTimeoutError
            _propagate_executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix="propagate")
            _propagate_future = _propagate_executor.submit(
            ...
            try:
                state, decision = _propagate_future.result(timeout=analysis_timeout_seconds)
            except FutureTimeoutError as timeout_err:
                logger.error(
                    f"❌ 分析超时: task_id={task_id}, symbol={request.stock_code}, timeout={analysis_timeout_seconds}s"
                )
                raise RuntimeError(
                    f"分析执行超时（>{analysis_timeout_seconds}秒）。请稍后重试，或降低研究深度/分析师数量。"
                ) from timeout_err
            finally:
                _propagate_executor.shutdown(wait=False, cancel_futures=True)
            ...
            try:
                if isinstance(decision, dict):
                    ...
                    raw_reasoning = decision.get('reasoning', '暂无分析推理')
                    # 对限流/降级等技术错误信息做友好化替换
                    if raw_reasoning and any(
                        k in str(raw_reasoning) for k in
                        ['RateLimitError', '429', '请查看日志', '已返回降级结果', 'TPM limit']
                    ):
                        raw_reasoning = (
                            "分析过程中遭遇 API 限流，已基于已有信息给出中性建议。"
                            "建议：1) 稍后重试 2) 使用较低分析级别（如标准/快速）"
                        )
                    formatted_decision = {
    ...
    async def get_task_status(self, task_id: str) -> Optional[Dict[str, Any]]:
        ...
        if result:
            ...
            # 优先从Redis获取详细进度信息（放入线程池，避免同步Redis I/O阻塞事件循环）
            redis_progress = await asyncio.to_thread(get_progress_by_id, task_id)
            if redis_progress:
    ...
    async def cleanup_zombie_tasks(self, max_running_hours: int = 2) -> Dict[str, Any]:
        ...
        try:
            ...
            # 更新为失败状态
            update_result = db.analysis_tasks.update_many(
                zombie_filter,
                {
                    "$set": {
                        "status": "failed",
    ...
    async def _save_analysis_result_web_style(self, task_id: str, result: Dict[str, Any]):
        ...
        try:
            ...
            result_insert = db.analysis_reports.insert_one(document)

            if result_insert.inserted_id:
                logger.info(f"✅ 分析报告已保存到PostgreSQL analysis_reports: {analysis_id}")
