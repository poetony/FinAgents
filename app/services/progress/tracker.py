...
class RedisProgressTracker:
    ...
    def _get_analyst_step_info(self, analyst: str) -> Dict[str, str]:
        ...
        # 未命中映射时：若已有「分析师」后缀则不重复添加
        suffix = "" if (isinstance(analyst, str) and analyst.endswith("分析师")) else "分析师"
        return mapping.get(analyst, {"name": f"🔍 {analyst}{suffix}", "description": f"进行{analyst}相关的专业分析"})
...
# 同步 Redis 客户端缓存（避免每次状态查询都新建连接，减少 I/O 延迟）
_progress_redis_client = None
_progress_redis_client_lock = __import__('threading').Lock()


def _get_progress_redis_client():
    """获取用于进度查询的同步 Redis 客户端（带超时、可复用）"""
    global _progress_redis_client
    with _progress_redis_client_lock:
        ...
        try:
            ...
            redis_password = os.getenv('REDIS_PASSWORD', None)
            redis_db = int(os.getenv('REDIS_DB', 0))
            # 关键：设置连接和读写超时，避免慢 I/O 阻塞（默认无超时会等很久）
            timeout_sec = int(os.getenv('REDIS_PROGRESS_TIMEOUT', '3'))
            kwargs = {
                "host": redis_host,
                "port": redis_port,
                "db": redis_db,
            ...
            _progress_redis_client = redis.Redis(**kwargs)
            _progress_redis_client.ping()
            logger.info(f"📊 [Redis进度] 进度查询 Redis 客户端已初始化 (timeout={timeout_sec}s)")
            return _progress_redis_client
        except Exception as e:
            logger.warning(f"📊 [Redis进度] 进度查询 Redis 初始化失败: {e}")
            return None


def get_progress_by_id(task_id: str) -> Optional[Dict[str, Any]]:
    """根据任务ID获取进度（使用复用 Redis 客户端 + 超时，避免慢 I/O）"""
    try:
        redis_enabled = os.getenv('REDIS_ENABLED', 'false').lower() == 'true'

        if redis_enabled:
            try:
                redis_client = _get_progress_redis_client()
                if redis_client:
