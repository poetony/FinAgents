...
class NewsDataService:
    ...
    def save_news_data_sync(
        ...
        try:
            ...
            # 执行批量操作（同步方式：PostgresCollectionProxy 需用 bulk_write_sync，MongoDB 用 bulk_write）
            if operations:
                if hasattr(collection, "bulk_write_sync"):
                    result = collection.bulk_write_sync(operations)
                else:
