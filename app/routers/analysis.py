...
def _ensure_datetime(val):
    """将字符串或 datetime 转为 naive datetime，避免 datetime - str 运算错误"""
    if val is None:
        return None
...
async def get_task_status_new(
    ...
    try:
        ...
        if result:
            ...
            # 内存中没有找到，尝试从PostgreSQL中查找
            logger.info(f"📊 [STATUS] 内存中未找到，尝试从PostgreSQL查找: {task_id}")

            from app.core.database import get_mongo_db_sync
            db = get_mongo_db_sync()

            # 首先从analysis_tasks集合中查找（正在进行的任务）
            task_result = db.analysis_tasks.find_one({"task_id": task_id})
            ...
            if task_result:
                ...
                # 计算时间信息（PostgreSQL JSONB 可能返回 ISO 字符串，需转为 datetime）
                ...
                err_msg = task_result.get("last_error") or task_result.get("error_message") or task_result.get("message")
                status_data = {
                    "task_id": task_id,
                    "status": status,
                    "progress": progress,
                    "message": err_msg if status == "failed" else f"任务{status}中...",
                    "current_step": task_result.get("current_step", status),
                    "error_message": err_msg if status == "failed" else None,
                    "last_error": task_result.get("last_error"),
            ...
            mongo_result = db.analysis_reports.find_one({"task_id": task_id})

            if mongo_result:
                logger.info(f"✅ [STATUS] 从analysis_reports找到任务: {task_id}")

                # 构造状态响应（模拟已完成的任务）
                # 计算已完成任务的时间信息（兼容字符串格式的日期）
                start_time = _ensure_datetime(mongo_result.get("created_at"))
                end_time = _ensure_datetime(mongo_result.get("updated_at"))
                elapsed_time = 0
...
async def get_task_result(
    ...
    try:
        ...
        if not result_data:
            # 内存中没有找到，尝试从PostgreSQL中查找
            logger.info(f"📊 [RESULT] 内存中未找到，尝试从PostgreSQL查找: {task_id}")
            ...
            if not mongo_result:
                ...
                tasks_doc_for_id = db.analysis_tasks.find_one({"task_id": task_id}, {"result.analysis_id": 1})
                ...
                if analysis_id:
                    ...
                    mongo_result = db.analysis_reports.find_one({"analysis_id": analysis_id})
            ...
            if mongo_result:
                ...
                tasks_doc = db.analysis_tasks.find_one(
                    {"task_id": task_id},
        ...
        if 'reports' not in result_data or not result_data['reports']:
            ...
            try:
                ...
                # 使用与 simple_analysis_service 一致的 project_root（TradingAgents）
                _analysis_file = Path(__file__).resolve()
                project_root = _analysis_file.parent.parent.parent
                if base_env:
                    base_path = Path(base_env)
                    if not base_path.is_absolute():
                        base_path = project_root / base_env
                else:
                    # 与保存逻辑一致：优先 Stock_analysis/results
                    parent_root = project_root.parent
                    stock_analysis_results = parent_root / "Stock_analysis" / "results"
                    base_path = stock_analysis_results if (parent_root / "Stock_analysis").exists() else (project_root / 'results')
...
async def delete_task(
    ...
    try:
        ...
        result = db.analysis_tasks.delete_one({"task_id": task_id})
