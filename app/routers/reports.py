...
def _build_report_query(report_id: str) -> Dict[str, Any]:
    # PostgreSQL JSONB 代理不支持 $or，返回简单 _id 查询
    return {"_id": report_id}


def _find_report_by_id(db, report_id: str) -> Optional[Dict]:
    """尝试多种 ID 字段查找报告"""
    doc = db.analysis_reports.find_one({"_id": report_id})
    if doc:
        return doc
    doc = db.analysis_reports.find_one({"analysis_id": report_id})
    if doc:
        return doc
    doc = db.analysis_reports.find_one({"task_id": report_id})
    if doc:
        return doc
    return None
...
async def get_reports_list(
    ...
    try:
        ...
        # 计算总数
        total = db.analysis_reports.count_documents(query)
...
async def get_report_detail(
    ...
    try:
        ...
        # 支持多种ID查询
        doc = _find_report_by_id(db, report_id)
...
async def get_report_module_content(
    ...
    try:
        ...
        doc = db.analysis_reports.find_one(query)
...
async def delete_report(
    ...
    try:
        ...
        # 查询报告（支持多种ID）
        result = db.analysis_reports.delete_one({"_id": report_id})
    ...
    try:
        ...
        if format == "json":
            ...
            if not report_exporter.weasyprint_available and not report_exporter.pdfkit_available:
                raise HTTPException(
                    status_code=400,
                    detail="PDF 导出功能不可用。请安装: pip install weasyprint"
                )
