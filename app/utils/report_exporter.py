...
# 检查 weasyprint（纯 Python PDF 引擎，无需系统依赖）
...
try:
    ...
    logger.info("✅ weasyprint 可用（PDF 生成引擎）")
except ImportError:
    WEASYPRINT_ERROR = "weasyprint 未安装"
...
"""
    ...
    def generate_pdf_report(self, report_doc: Dict[str, Any]) -> bytes:
        ...
        优先级: weasyprint > pdfkit + wkhtmltopdf
        weasyprint 是纯 Python 方案，无需额外系统依赖
        """
        logger.info("📊 开始生成 PDF 文档...")

        # 检查是否有任一 PDF 引擎可用
        if not self.pdfkit_available and not self.weasyprint_available:
            error_msg = (
                "PDF 导出功能不可用。请安装以下任一方案:\n\n"
                "方案1（推荐-纯Python）: pip install weasyprint\n"
                "方案2: pip install pdfkit && 安装 wkhtmltopdf\n"
    ...
    def _generate_pdf_with_weasyprint(self, html_content: str) -> bytes:
        """使用 weasyprint 生成 PDF（纯 Python，无需系统依赖）"""
        import weasyprint
