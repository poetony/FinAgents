...
# 导入 PostgreSQL 缓存适配器
from .cache.mongodb_cache_adapter import get_postgres_cache_adapter, get_stock_data_with_fallback, get_financial_data_with_fallback
...
class OptimizedChinaDataProvider:
    ...
    def _format_financial_data_to_fundamentals(self, financial_data: Dict[str, Any], symbol: str) -> str:
        """将 PostgreSQL 财务数据转换为基本面分析格式"""
        try:
            # 提取关键财务指标（兼容 revenue/net_income 与 total_revenue/net_profit）
            revenue = financial_data.get('revenue') or financial_data.get('total_revenue', 'N/A')
            net_profit = financial_data.get('net_income') or financial_data.get('net_profit', 'N/A')
...
"""
    ...
    def get_stock_data(self, symbol: str, start_date: str, end_date: str,
        ...
        # 1. 优先尝试从 PostgreSQL 获取（如果启用了 TA_USE_APP_CACHE）
        if not force_refresh:
            ...
            if adapter.use_app_cache:
                ...
                if df is not None and not df.empty:
                    logger.info(f"📊 [数据来源: PostgreSQL] 使用 PostgreSQL 历史数据: {symbol} ({len(df)}条记录)")
                    return df.to_string()
    ...
    def get_fundamentals_data(self, symbol: str, force_refresh: bool = False) -> str:
        ...
        # 1. 数据库可用时，直接从 stock_financial_data 获取（mairui/csmar 等预导入数据）
        if not force_refresh:
            try:
                from tradingagents.config.database_manager import get_database_manager
                dm = get_database_manager()
                if dm.is_db_available():
                    client = dm.get_db_client()
                    if client:
                        db = client.get_database('tradingagents')
                        code6 = symbol.replace('.SH', '').replace('.SZ', '').zfill(6)
                        for ds in ['mairui', 'csmar', 'tushare', 'akshare', 'baostock']:
                            doc = db.stock_financial_data.find_one(
                                {"code": code6, "data_source": ds},
                                {"_id": 0},
                                sort=[("report_period", -1)]
                            )
                            if doc:
                                logger.info(f"💰 [数据来源: 数据库-{ds}] 使用财务数据: {symbol}")
                                return self._format_financial_data_to_fundamentals(doc, symbol)
            except Exception as e:
                logger.debug(f"从数据库获取基本面失败: {e}")
...
- **净利润**: {financial_estimates.get('net_income', 'N/A')}
- **总资产**: {financial_estimates.get('total_assets', 'N/A')}
- **股东权益**: {financial_estimates.get('total_equity', 'N/A')}
...
"""
    ...
    def _get_real_financial_metrics(self, symbol: str, price_value: float) -> dict:
        ...
        try:
            ...
            if db_manager.is_db_available():
                try:
                    db_client = db_manager.get_db_client()
                    db = db_client.get_database('tradingagents') if db_client else None
            ...
            # 第一优先级：数据库可用时，直接从 stock_financial_data 获取（mairui/csmar 等预导入数据）
            financial_data = None
            if db_manager.is_db_available():
                try:
                    _client = db_manager.get_db_client()
                    db = _client.get_database('tradingagents') if _client else None
                    if db:
                        coll = db.stock_financial_data
                        # 数据源优先级：mairui > csmar > tushare > akshare > baostock
                        for data_source in ['mairui', 'csmar', 'tushare', 'akshare', 'baostock']:
                            doc = coll.find_one(
                            ...
                            if doc:
                                financial_data = doc
                                logger.info(f"✅ [财务数据] 从 stock_financial_data 获取 {symbol} (数据源: {data_source})")
                                break
                except Exception as e:
                    logger.warning(f"⚠️ 从数据库获取财务数据失败: {e}")

            if financial_data:
                metrics = self._parse_postgres_financial_data(financial_data, price_value)
                if metrics:
                    return metrics
                logger.warning(f"⚠️ 财务数据解析失败: {symbol}")

            if not financial_data:
                logger.info(f"🔄 数据库中未找到{symbol}财务数据，尝试从 AKShare API 获取")

            # 辅助函数：在新线程中运行异步协程（避免与已运行的事件循环冲突）
            def _run_async_in_thread(coro_factory):
                """在独立线程中创建新事件循环并运行协程，避免 RuntimeError: This event loop is already running"""
                import asyncio
            ...
            if akshare_provider.connected:
                logger.info(f"🔄 通过独立线程调用 AKShare 获取 {symbol} 财务数据")
                try:
                    financial_data = _run_async_in_thread(
                        lambda: akshare_provider.get_financial_data(symbol)
                    )
                except Exception as _ae:
                    logger.warning(f"⚠️ AKShare异步调用失败: {_ae}")
                    financial_data = {}
                ...
                if financial_data and any(not v.empty if hasattr(v, 'empty') else bool(v) for v in financial_data.values()):
                    ...
                    try:
                        stock_info = _run_async_in_thread(
                            lambda: akshare_provider.get_stock_basic_info(symbol)
                        )
                    except Exception:
                        stock_info = None
            ...
            try:
                financial_data = _run_async_in_thread(
                    lambda: provider.get_financial_data(symbol)
                )
            except Exception as _te:
                logger.warning(f"⚠️ Tushare异步调用失败: {_te}")
                return None
    ...
    def _parse_postgres_financial_data(self, financial_data: dict, price_value: float) -> dict:
        ...
        try:
            ...
            revenue = _safe_float(latest_indicators.get('revenue') or latest_indicators.get('营业收入'))
            net_income = _safe_float(latest_indicators.get('net_income') or latest_indicators.get('净利润'))
            total_assets = _safe_float(latest_indicators.get('total_assets') or latest_indicators.get('总资产'))
            total_equity = _safe_float(latest_indicators.get('total_equity') or latest_indicators.get('股东权益合计'))

            # 营收、净利润、总资产、股东权益 - 供报告使用
            if revenue is not None:
                metrics["revenue"] = f"{revenue/1e8:.2f}亿元" if revenue >= 1e8 else f"{revenue/1e4:.2f}万元"
            if net_income is not None:
                metrics["net_income"] = f"{net_income/1e8:.2f}亿元" if abs(net_income) >= 1e8 else f"{net_income/1e4:.2f}万元"
            if total_assets is not None:
                metrics["total_assets"] = f"{total_assets/1e8:.2f}亿元" if total_assets >= 1e8 else f"{total_assets/1e4:.2f}万元"
            if total_equity is not None:
                metrics["total_equity"] = f"{total_equity/1e8:.2f}亿元" if total_equity >= 1e8 else f"{total_equity/1e4:.2f}万元"

            debt_ratio = latest_indicators.get('debt_to_assets') or latest_indicators.get('资产负债率') or latest_indicators.get('debt_ratio')
            if debt_ratio is not None and str(debt_ratio) not in ('nan', '--'):
                try:
                    dr_val = float(debt_ratio)
                    metrics["debt_ratio"] = f"{dr_val:.1f}%" if 0 <= dr_val <= 100 else "N/A"
                except (ValueError, TypeError):
                    metrics["debt_ratio"] = "N/A"
            else:
                metrics["debt_ratio"] = "N/A"

            # ROE - 优先直接值，否则用 净利润/股东权益 计算
            roe = latest_indicators.get('roe') or latest_indicators.get('roe_waa')
            if roe is not None and str(roe) not in ('nan', '--'):
                try:
                    roe_val = float(roe)
                    metrics["roe"] = f"{roe_val:.1f}%" if -200 <= roe_val <= 200 else "N/A"
                except (ValueError, TypeError):
                    metrics["roe"] = "N/A"
            elif net_income is not None and total_equity is not None and total_equity != 0:
                roe_val = (net_income / total_equity) * 100
                metrics["roe"] = f"{roe_val:.1f}%" if -200 <= roe_val <= 200 else "N/A"
            else:
                metrics["roe"] = "N/A"

            # ROA - 优先直接值，否则用 净利润/总资产 计算
            roa = latest_indicators.get('roa') or latest_indicators.get('roa2')
            if roa is not None and str(roa) not in ('nan', '--'):
                try:
                    roa_val = float(roa)
                    metrics["roa"] = f"{roa_val:.1f}%" if -100 <= roa_val <= 100 else "N/A"
                except (ValueError, TypeError):
                    metrics["roa"] = "N/A"
            elif net_income is not None and total_assets is not None and total_assets != 0:
                roa_val = (net_income / total_assets) * 100
                metrics["roa"] = f"{roa_val:.1f}%" if -100 <= roa_val <= 100 else "N/A"
            else:
                metrics["roa"] = "N/A"
            ...
            if gross_margin is not None and str(gross_margin) not in ('nan', '--'):
                try:
                    gm_val = float(gross_margin)
                    metrics["gross_margin"] = f"{gm_val:.1f}%" if -100 <= gm_val <= 100 else "N/A"
                except (ValueError, TypeError):
                    metrics["gross_margin"] = "N/A"
            else:
                metrics["gross_margin"] = "N/A"

            # 净利率 - 优先直接值，否则用 净利润/营业收入 计算
            net_margin = latest_indicators.get('netprofit_margin') or latest_indicators.get('net_margin')
            if net_margin is not None and str(net_margin) not in ('nan', '--'):
                try:
                    nm_val = float(net_margin)
                    metrics["net_margin"] = f"{nm_val:.1f}%" if -100 <= nm_val <= 100 else "N/A"
                except (ValueError, TypeError):
                    metrics["net_margin"] = "N/A"
            elif net_income is not None and revenue is not None and revenue != 0:
                nm_val = (net_income / revenue) * 100
                metrics["net_margin"] = f"{nm_val:.1f}%" if -100 <= nm_val <= 100 else "N/A"
            else:
                metrics["net_margin"] = "N/A"

            # 估值指标：优先使用 stock_financial_data 中预导入的 total_mv/pe_ttm/pb（mairui/csmar 等）
            pe_value = None
            pe_ttm_value = None
            pb_value = None
            is_loss_stock = False  # 🔥 标记是否为亏损股

            # 🔥 第0层：优先使用 stock_financial_data 中的估值指标（mairui/csmar 下载的预导入数据）
            total_mv_from_db = _safe_float(latest_indicators.get('total_mv'))
            pe_ttm_from_db = _safe_float(latest_indicators.get('pe_ttm'))
            pb_from_db = _safe_float(latest_indicators.get('pb'))
            if total_mv_from_db is not None and total_mv_from_db > 0:
                metrics["total_mv"] = f"{total_mv_from_db:.2f}亿元"
                logger.info(f"✅ [总市值-第0层] 使用 stock_financial_data: {total_mv_from_db:.2f}亿元")
            if pe_ttm_from_db is not None and pe_ttm_from_db > 0:
                pe_value = pe_ttm_from_db
                pe_ttm_value = pe_ttm_from_db
                metrics["pe"] = f"{pe_ttm_from_db:.1f}倍"
                metrics["pe_ttm"] = f"{pe_ttm_from_db:.1f}倍"
                logger.info(f"✅ [PE/PE_TTM-第0层] 使用 stock_financial_data: {pe_ttm_from_db:.1f}倍")
            elif pe_ttm_from_db is not None and (pe_ttm_from_db <= 0 or str(pe_ttm_from_db) == 'nan'):
                is_loss_stock = True
            if pb_from_db is not None and pb_from_db > 0:
                pb_value = pb_from_db
                metrics["pb"] = f"{pb_from_db:.2f}倍"
                logger.info(f"✅ [PB-第0层] 使用 stock_financial_data: {pb_from_db:.2f}倍")

            # 若第0层已取全，跳过实时计算
            has_valuation_from_db = (
                "total_mv" in metrics and metrics["total_mv"] != "N/A"
                and pe_ttm_value is not None
                and pb_value is not None
            )

            try:
                # 第1层：若 stock_financial_data 无完整估值，再尝试实时计算
                if not has_valuation_from_db:
                    from tradingagents.dataflows.realtime_metrics import get_pe_pb_with_fallback
                    from tradingagents.config.database_manager import get_database_manager

                    db_manager = get_database_manager()
                    if db_manager.is_db_available():
                        client = db_manager.get_db_client()
                        ...
                        if stock_code:
                            ...
                            if realtime_metrics:
                                ...
                                if pe_ttm_value is not None and pe_ttm_value > 0:
                                    ...
                                    logger.info(f"✅ [PE_TTM计算-第1层成功] PE_TTM={pe_ttm_value:.2f}倍 | 来源={realtime_metrics.get('source', 'unknown')} | 实时={is_realtime}")
            ...
            logger.info(f"✅ PostgreSQL 财务数据解析成功: ROE={metrics.get('roe')}, ROA={metrics.get('roa')}, 毛利率={metrics.get('gross_margin')}, 净利率={metrics.get('net_margin')}")
            return metrics

        except Exception as e:
            logger.error(f"❌ PostgreSQL 财务数据解析失败: {e}", exc_info=True)
            return None
    ...
    def _analyze_valuation(self, financial_estimates: dict) -> str:
        """分析估值水平"""
        valuation_score = financial_estimates.get('valuation_score', 5)
    ...
    def _analyze_risks(self, symbol: str, financial_estimates: dict, industry_info: dict) -> str:
        """分析投资风险"""
        risk_level = financial_estimates.get('risk_level', '未知')
...
- 政策调整风险"""
    ...
    def _generate_investment_advice(self, financial_estimates: dict, industry_info: dict) -> str:
        """生成投资建议"""
        def _safe_score(k: str) -> float:
            v = financial_estimates.get(k, 5)
            return float(v) if isinstance(v, (int, float)) else 5.0

        total_score = (_safe_score('fundamental_score') + _safe_score('valuation_score') + _safe_score('growth_score')) / 3
...
def _add_financial_cache_methods():
    ...
    def _get_cached_raw_financial_data(self, symbol: str) -> dict:
        ...
        try:
            ...
            if not client:
                logger.debug(f"📊 [财务缓存] PostgreSQL 客户端不可用")
                return None
