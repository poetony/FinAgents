...
def calculate_realtime_pe_pb(
    ...
    Args:
        symbol: 6位股票代码
        db_client: 数据库客户端（PostgreSQL，可选）
    ...
    try:
        ...
        if db_client is None:
            ...
            if not db_manager.is_db_available():
                logger.debug("数据库不可用，无法计算实时PE/PB")
                return None
        ...
        logger.info(f"🔍 [数据库查询] 查询条件: code={code6}, source=tushare")
        ...
        if not basic_info:
            ...
            logger.warning(f"   数据库中该股票的数据源: {[s.get('source') for s in all_sources]}")
        ...
        # 8. 获取财务数据（用于计算 PB），按数据源优先级：mairui > csmar > tushare > akshare > baostock
        financial_data = None
