...
class RealtimeNewsAggregator:
    ...
    def _deduplicate_news(self, news_items: List[NewsItem]) -> List[NewsItem]:
        ...
        for item in news_items:
            ...
            # 检查标题长度（仅过滤真正无效的：空或极短，避免误删中文短标题）
            if len(title_key) <= 2:
...
def get_realtime_stock_news(ticker: str, curr_date: str, hours_back: int = 6) -> str:
    ...
    if '.' in ticker:
        ...
        try:
            ...
            safe_market_info = dict(market_info)
            # 控制台在 GBK 编码下可能无法输出某些货币符号，统一替换为 ASCII 文本
            if "currency_symbol" in safe_market_info:
                safe_market_info["currency_symbol"] = "CNY"
            logger.info(f"[新闻分析] StockUtils返回市场信息: {safe_market_info}")
    ...
    # 备用方案2: 对于A股，再次尝试东方财富（避免先等 Google 超时约 87 秒）
    if is_china_stock:
        logger.info(f"[新闻分析] 备用方案：A股再次尝试东方财富，再试 Google")
        try:
            from tradingagents.dataflows.providers.china.akshare import AKShareProvider
            ...
            clean_ticker = ticker.replace('.SH', '').replace('.SZ', '').replace('.SS', '')\
                            .replace('.XSHE', '').replace('.XSHG', '')
            logger.info(f"[新闻分析] 开始从东方财富获取 {clean_ticker} 的新闻数据（备用）")
            start_time = datetime.now(ZoneInfo(get_timezone_name()))
            news_df = provider.get_stock_news_sync(symbol=clean_ticker, limit=10)
            end_time = datetime.now(ZoneInfo(get_timezone_name()))
            time_taken = (end_time - start_time).total_seconds()
            ...
            if news_df is not None and hasattr(news_df, "empty") and not news_df.empty:
                news_count = len(news_df)
                logger.info(f"[新闻分析] 备用东方财富成功获取 {news_count} 条新闻，耗时 {time_taken:.2f} 秒")
                ...
                for _, row in news_df.iterrows():
                    ...
                    report += f"🔗 {row.get('新闻链接', '')}\n\n"
                    report += f"{row.get('新闻内容', '无内容')}\n\n"
                logger.info(f"[新闻分析] 成功生成东方财富新闻报告（备用），新闻来源: 东方财富")
                return report
            else:
                logger.warning(f"[新闻分析] 备用东方财富未获取到 {clean_ticker} 的新闻，耗时 {time_taken:.2f} 秒，尝试 Google")
        except Exception as e:
            logger.error(f"[新闻分析] 备用东方财富新闻获取失败: {e}，将尝试 Google 新闻")
