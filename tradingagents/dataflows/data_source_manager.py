...
class ChinaDataSource(Enum):
    """
    中国股票数据源枚举
    优先级：PostgreSQL(quantdb) -> AKShare -> Tushare
    """
    QUANTDB = "quantdb"  # quantdb 数据库（PostgreSQL, daily_bars, securities）
    LOCAL = "local"  # 用户导入的本地数据（pg_doc 表）
...
class USDataSource(Enum):
    """美股数据源枚举（MONGODB 已弃用，仅保留兼容）"""
    MONGODB = "mongodb"  # 已弃用，不再使用
...
class DataSourceManager:
    ...
    def __init__(self):
        """初始化数据源管理器，优先级：PostgreSQL(quantdb) -> AKShare -> Tushare"""
        self.default_source = ChinaDataSource.QUANTDB
        self.available_sources = [ChinaDataSource.QUANTDB, ChinaDataSource.AKSHARE, ChinaDataSource.TUSHARE]
        self.current_source = self.default_source

        self.cache_manager = None
        self.cache_enabled = False

        logger.info(f"📊 数据源管理器初始化完成（PostgreSQL→AKShare→Tushare）")
        logger.info(f"   优先级: quantdb.daily_bars -> akshare -> tushare")

    def _get_data_source_priority_order(self, symbol: Optional[str] = None) -> List[ChinaDataSource]:
        """
        获取数据源优先级顺序（用于降级）
        固定顺序：PostgreSQL(quantdb) -> AKShare -> Tushare
        """
    ...
    def _get_default_source(self) -> ChinaDataSource:
        """获取默认数据源"""
        return ChinaDataSource.QUANTDB
    ...
    def get_fundamentals_data(self, symbol: str) -> str:
        ...
        try:
            # 根据数据源调用相应的获取方法（已移除 MongoDB）
            if self.current_source == ChinaDataSource.LOCAL:
    ...
    def get_data_adapter(self):
        ...
        # TDX 已移除
        elif self.current_source == ChinaDataSource.QUANTDB:
            from .providers.china.quantdb_provider import get_quantdb_provider
            return get_quantdb_provider()
        else:
    ...
    def get_stock_dataframe(self, symbol: str, start_date: str = None, end_date: str = None, period: str = "daily") -> pd.DataFrame:
        ...
        try:
            # 按优先级尝试各数据源
            df = None
            for source in self._get_data_source_priority_order(symbol):
                try:
                    if source == ChinaDataSource.QUANTDB:
                        from .providers.china.quantdb_provider import get_stock_data as qdb_get
                        df = qdb_get(symbol, start_date, end_date)
                    ...
                    if df is not None and not df.empty:
                        logger.info(f"✅ [DataFrame接口] 从 {source.value} 获取成功: {len(df)}条")
    ...
    def get_stock_data(self, symbol: str, start_date: str = None, end_date: str = None, period: str = "daily") -> str:
        ...
        try:
            # 按优先级尝试：PostgreSQL(quantdb) -> AKShare -> Tushare
            result = None
            actual_source = None
            for source in self._get_data_source_priority_order(symbol):
                try:
                    if source == ChinaDataSource.QUANTDB:
                        result = self._get_quantdb_data(symbol, start_date, end_date, period)
                        actual_source = "quantdb"
                    elif source == ChinaDataSource.AKSHARE:
    ...
    def _get_quantdb_data(self, symbol: str, start_date: str, end_date: str, period: str = "daily") -> str:
        """
        从 quantdb.daily_bars 获取 A 股日线数据（仅支持 daily 周期）
        """
        if period != "daily":
            logger.warning(f"[QuantDB] 仅支持 daily 周期，忽略 period={period}")
        try:
            from .providers.china.quantdb_provider import get_stock_data as qdb_get_data
            from .providers.china.quantdb_provider import get_stock_info as qdb_get_info

            df = qdb_get_data(symbol, start_date, end_date)
            if df is None or df.empty:
                return f"❌ 无法获取股票 {symbol} 的历史数据\n\n💡 请检查 quantdb.daily_bars 中是否有该股票数据"

            info = qdb_get_info(symbol)
            stock_name = (info.get("name") or f"股票{symbol}") if info else f"股票{symbol}"

            result = self._format_stock_data_response(df, symbol, stock_name, start_date, end_date)
            logger.info(f"✅ [QuantDB] 获取 {symbol} 数据成功: {len(df)} 条")
            return result
        except Exception as e:
    ...
    def _get_local_data(self, symbol: str, start_date: str, end_date: str, period: str = "daily") -> str:
        """
        从 PostgreSQL pg_doc_stock_daily_quotes 获取 local 数据源的历史数据
    ...
    def get_stock_info(self, symbol: str) -> Dict:
        ...
        logger.info(f"📊 [QuantDB] 获取股票信息: {symbol}")
        try:
            from .providers.china.quantdb_provider import get_stock_info as qdb_get_info
            info = qdb_get_info(symbol)
            if info and info.get("name"):
                ...
                logger.info(f"✅ [QuantDB] 获取股票信息成功: {symbol} - {result['name']}")
                return result
            return {"symbol": symbol, "name": f"股票{symbol}", "source": "quantdb"}
        except Exception as e:
            logger.error(f"❌ [QuantDB] 获取股票信息失败: {symbol}, {e}", exc_info=True)
            return {"symbol": symbol, "name": f"股票{symbol}", "source": "quantdb", "error": str(e)}
    ...
    def get_stock_basic_info(self, stock_code: str = None) -> Optional[Dict[str, Any]]:
        ...
        if stock_code is None:
            # 返回所有股票列表（从 PostgreSQL pg_doc_stock_basic_info）
    ...
    def _try_fallback_stock_info(self, symbol: str) -> Dict:
        ...
        # 获取所有可用数据源（排除当前）
        sources_to_try = [s for s in self.available_sources if s != self.current_source]
    ...
    def _get_local_fundamentals(self, symbol: str) -> str:
        """
        从 PostgreSQL pg_doc_stock_basic_info 获取 local 数据源的基本面数据
...
"""
    ...
    def _get_valuation_indicators(self, symbol: str) -> Dict:
        """从 PostgreSQL pg_doc_stock_basic_info 获取估值指标"""
        try:
            from app.core.database import get_mongo_db_sync
            db = get_mongo_db_sync()
            result = db.stock_basic_info.find_one({'ts_code': symbol}) or db.stock_basic_info.find_one({'symbol': symbol})
            if result:
                return {
                    'pe': result.get('pe'),
                    'pb': result.get('pb'),
    ...
    def _generate_fundamentals_analysis(self, symbol: str) -> str:
        ...
        try:
            ...
            report += "💡 建议: 确保 PostgreSQL 中有完整的财务数据\n"
...
def get_china_stock_data_unified(symbol: str, start_date: str, end_date: str) -> str:
    ...
    if result and "❌" not in result:
        try:
            from .providers.china.sector_provider import get_stock_sectors, get_sector_analysis, format_sector_context
            sectors = get_stock_sectors(symbol)
            if sectors.get("sw1") or sectors.get("sw2"):
                sector_analysis = get_sector_analysis(
                    date=end_date,
                    sw1=sectors.get("sw1"),
                    sw2=sectors.get("sw2"),
                )
                sector_text = format_sector_context(sector_analysis, sectors)
                if sector_text:
                    result = result + "\n\n" + sector_text
                    logger.info(f"[Sector] 行业分析已附加到 {symbol} 数据中 ({len(sector_text)}字符)")
        except Exception as e:
            logger.warning(f"[Sector] 附加行业分析失败: {e}")
...
class USDataSourceManager:
    """
    美股数据源管理器（已移除 MongoDB）
    支持：yfinance, alpha_vantage, finnhub
    """
    ...
    def _get_enabled_sources_from_db(self) -> List[str]:
        ...
        try:
            ...
            # 🔥 若数据库无配置，使用默认顺序（yfinance 免费无需 Key，优先）
            if not groupings:
                logger.info("ℹ️ 数据库无美股数据源配置，使用默认: yfinance, alpha_vantage, finnhub")
