...
class AKShareProvider(BaseStockDataProvider):
    ...
    def _get_stock_news_direct(self, symbol: str, limit: int = 10) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            # 标准化股票代码（兼容 000536 / 000536.SZ / 关键词）
            symbol_input = str(symbol).strip().upper()
            symbol_clean = (
                symbol_input.replace(".SH", "")
                .replace(".SZ", "")
                .replace(".SS", "")
                .replace(".XSHE", "")
                .replace(".XSHG", "")
            )
            symbol_6 = symbol_clean.zfill(6) if symbol_clean.isdigit() else symbol_clean
            ...
            param = {
                ...
                "param": {
                    "cmsArticleWebOld": {
                        ...
                        "sort": "pubtime",
                        "pageIndex": 1,
                        "pageSize": min(max(int(limit or 10), 1), 100),
                        "preTag": "<em>",
                        "postTag": "</em>"
                    }
                }
            ...
            response = curl_requests.get(
                ...
                headers={"Referer": "https://so.eastmoney.com/news/s"}
            )
            ...
            for article in articles:
                news_data.append({
                    ...
                    "关键词": symbol_6,
                    "文章来源": article.get("source", "东方财富网"),
                    "新闻类型": article.get("type", "")
                })

            df = self._normalize_stock_news_em_df(pd.DataFrame(news_data), limit=limit)
            if df is None:
                return None
    ...
    def _normalize_stock_news_em_df(self, news_df: Optional[pd.DataFrame], limit: int = 10) -> Optional[pd.DataFrame]:
        """
        标准化 stock_news_em 返回字段，统一为 6 列:
        ...
        field_candidates = {
            "关键词": ["关键词", "keyword", "symbol"],
            "新闻标题": ["新闻标题", "标题", "title"],
            "新闻内容": ["新闻内容", "内容", "摘要", "content", "summary"],
            "发布时间": ["发布时间", "时间", "date", "publish_time"],
            "文章来源": ["文章来源", "新闻来源", "来源", "source"],
            "新闻链接": ["新闻链接", "链接", "url"],
        }

        normalized = pd.DataFrame(index=news_df.index)
        for target_col, candidates in field_candidates.items():
            source_col = next((col for col in candidates if col in news_df.columns), None)
            if source_col:
                normalized[target_col] = news_df[source_col]
            else:
    ...
    async def _get_stock_list_cached(self):
        ...
        def fetch_stock_list():
            try:
                ...
                if "Excel file format cannot be determined" in error_msg or "engine manually" in error_msg:
                    logger.warning(f" Excel 格式识别失败，尝试指定引擎: {e}")
                    try:
                        # 尝试使用 openpyxl 引擎
                        import pandas as pd
                        ...
                        # 所以这里先记录警告，不影响主流程
                        logger.warning(f" 无法自动修复 Excel 格式问题，股票列表获取失败，但不影响其他功能")
                        return None
        ...
        try:
            ...
            if stock_list is not None and not stock_list.empty:
                ...
                logger.warning(f" 股票列表为空，可能是接口返回格式问题")
        except Exception as e:
    ...
    async def _get_stock_info_detail(self, code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f" 获取{code}个股详细信息失败: {e}")

            #  如果获取失败，返回基本信息（使用默认名称）
            # 注意：在实际使用中，股票名称应该从数据库中获取，这里只是作为最后的回退
            logger.debug(f" 无法获取{code}的详细信息，使用默认信息")
    ...
    async def _get_all_market_spot_data(self) -> Optional[pd.DataFrame]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.warning(f" 获取全市场快照失败 (尝试 {attempt + 1}/{max_retries}): {e}")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if is_connection_error and attempt < max_retries - 1:
                    ...
                    logger.warning(f" 获取{code}实时行情连接失败 (尝试 {attempt + 1}/{max_retries}): {error_str}")
                    ...
                    if attempt >= max_retries - 1:
                        ...
                        try:
                            ...
                            if target_quotes:
                                ...
                                logger.warning(f" 批量接口中未找到 {code} 的行情数据")
                                return None
    ...
    async def get_historical_data(
        ...
        try:
            ...
            if days_diff > 730:
                logger.info(f" {code} 时间跨度过大({days_diff}天)，分批获取数据...")
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if "shutdown" in str(runtime_error).lower() or "closed" in str(runtime_error).lower():
                        logger.warning(f" [AKShare API调用] 事件循环已关闭，停止获取数据: {code}")
                        self._event_loop_closed = True
                        return None
                    # 其他 RuntimeError，继续重试
                    if attempt < max_retries - 1:
                        wait_time = (attempt + 1) * 2
                        logger.warning(f" {code} 第{attempt + 1}次尝试失败（RuntimeError），{wait_time}秒后重试: {runtime_error}")
                        await asyncio.sleep(wait_time)
                    else:
                        raise
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.warning(f" {code} 第{attempt + 1}次尝试失败，{wait_time}秒后重试: {retry_error}")
                        await asyncio.sleep(wait_time)
                    else:
                        raise
            ...
            if "shutdown" in error_msg.lower() or "event loop is closed" in error_msg.lower():
                logger.warning(f" [AKShare] 事件循环已关闭，停止获取 {code} 的历史数据")
                self._event_loop_closed = True
                return None
    ...
    async def _get_historical_data_in_batches(
        ...
        try:
            ...
            if all_data:
                ...
                logger.warning(f" {code} 分批获取未获得任何数据")
                return None
    ...
    def get_stock_news_sync(self, symbol: str = None, limit: int = 10) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if symbol:
                ...
                # 优先使用直接接口，绕过 AKShare 在部分环境下的正则转义问题
                news_df = self._get_stock_news_direct(symbol=symbol_6, limit=limit)
                news_df = self._normalize_stock_news_em_df(news_df, limit=limit)
                if news_df is not None and not news_df.empty:
                    self.logger.info(f" {symbol} 直接接口新闻获取成功: {len(news_df)} 条")
                    return news_df
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        if attempt < max_retries - 1:
                            ...
                            return self._get_stock_news_direct(symbol=symbol_6, limit=limit)
                    except Exception as e:
                        err_text = str(e)
                        if "invalid escape sequence: \\u" in err_text:
                            self.logger.warning(f" {symbol} stock_news_em 正则转义异常，切换到直接接口")
                            return self._get_stock_news_direct(symbol=symbol_6, limit=limit)
                        if attempt < max_retries - 1:
                            ...
                            self.logger.error(f" {symbol} stock_news_em 获取失败，切换直接接口: {e}")
                            return self._get_stock_news_direct(symbol=symbol_6, limit=limit)
