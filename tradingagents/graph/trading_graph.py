...
class TradingAgentsGraph:
    ...
    def __init__(
        ...
        if quick_provider and deep_provider and quick_provider != deep_provider:
            ...
            self.deep_thinking_llm = ChatOpenAI(
                model=self.config["deep_think_llm"],
                base_url=self.config["backend_url"],
                api_key="ollama",  # Ollama 不需要真实 API Key，但 LangChain 要求非空
                temperature=deep_temperature,
                max_tokens=deep_max_tokens,
                timeout=deep_timeout
            )
        ...
        # Attach global LLM rate control and unified 429 retry.
        quick_cfg = self.config.get("quick_model_config", {}) or {}
        deep_cfg = self.config.get("deep_model_config", {}) or {}
        quick_provider_name = self.config.get("quick_provider") or self.config.get(
            "llm_provider", "unknown"
        )
        deep_provider_name = self.config.get("deep_provider") or self.config.get(
            "llm_provider", "unknown"
        )
        self.quick_thinking_llm = attach_rate_control_to_llm(
            self.quick_thinking_llm,
            provider=quick_provider_name,
            model=self.config.get("quick_think_llm", "unknown"),
            role="quick",
            retry_times=int(quick_cfg.get("retry_times", 3)),
        )
        self.deep_thinking_llm = attach_rate_control_to_llm(
            self.deep_thinking_llm,
            provider=deep_provider_name,
            model=self.config.get("deep_think_llm", "unknown"),
            role="deep",
            retry_times=int(deep_cfg.get("retry_times", 3)),
        )
    ...
    def propagate(self, company_name, trade_date, progress_callback=None, task_id=None, agent_context: Dict[str, Any] = None):
        ...
        # 处理决策并添加模型信息
        # skip_risk_stage 时 final_trade_decision 为空，使用 Trader 输出
        final_signal = final_state.get("final_trade_decision") or ""
        if not final_signal and self.config.get("skip_risk_stage"):
            final_signal = (
