...
class StockDataPreparer:
    ...
    def _check_database_data(self, stock_code: str, start_date: str, end_date: str) -> Dict:
        ...
        """
        try:
            # 使用 PostgreSQL(quantdb)/AKShare/Tushare 数据源，直接由 get_china_stock_data_unified 取数
            import os
            if os.getenv("QUANTDB_URL") or os.getenv("QUANTDB_DATABASE"):
                return {
                    ...
                    "record_count": 0,
                    "latest_date": None,
                    "message": "使用 QuantDB 数据源，跳过数据库预检"
                }

            # 无 quantdb 时仍假定可通过 akshare/tushare 获取，跳过预检
            return {
                "has_data": True,
                "is_latest": True,
                "record_count": 0,
                "latest_date": None,
                "message": "将尝试从 AKShare/Tushare 获取数据"
            }
    ...
    async def _get_independent_sync_service(data_source: str):
        ...
        try:
            ...
            # 使用 PostgreSQL（MongoDB 已移除）
            client = get_postgres_client_compat()
            ...
            logger.error(f"❌ 创建同步服务失败: {e}", exc_info=True)
            raise
