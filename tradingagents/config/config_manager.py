...
# MongoDB 已移除，Token 使用记录使用 JSON 文件存储
...
class ConfigManager:
    ...
    def __init__(self, config_dir: str = "config"):
        # 优先使用环境变量，确保分析流程使用 Stock_analysis 配置（data_dir 等）
        env_config = os.getenv("STOCK_ANALYSIS_CONFIG_DIR") or os.getenv("TRADINGAGENTS_CONFIG_DIR")
        if env_config and env_config.strip():
            self.config_dir = Path(env_config.strip())
        else:
            self.config_dir = Path(config_dir)
        if not self.config_dir.exists():
            self.config_dir = Path(config_dir)  # 回退到默认
        ...
        # Token 使用记录使用 JSON 文件（MongoDB 已移除）
    ...
    def _load_env_file(self):
        ...
        优先使用 STOCK_ANALYSIS_ENV_FILE 或从 STOCK_ANALYSIS_CONFIG_DIR 推断的 .env，
        便于 Stock_analysis 等独立项目使用自己的环境变量，与 TradingAgents 严格分离。
        """
        env_file = None
        env_path = os.getenv("STOCK_ANALYSIS_ENV_FILE")
        if env_path and env_path.strip():
            env_file = Path(env_path.strip())
        else:
            config_dir = os.getenv("STOCK_ANALYSIS_CONFIG_DIR") or os.getenv("TRADINGAGENTS_CONFIG_DIR")
            if config_dir and config_dir.strip():
                # 从 config 目录推断项目根：config_dir 的父目录下的 .env
                env_file = Path(config_dir.strip()).parent / ".env"
        if not env_file or not env_file.exists():
            # 默认：TradingAgents 项目根目录
    ...
    def add_usage_record(self, provider: str, model_name: str, input_tokens: int,
        ...
        #  详细日志：记录保存位置
        logger.info(f" [Token记录] 准备保存: {provider}/{model_name}, 输入={input_tokens}, 输出={output_tokens}, 成本=RMB {cost:.4f}, session={session_id}")
...
class TokenTracker:
    ...
    def _check_cost_alert(self, current_cost: float):
        ...
        if total_today >= threshold:
            logger.warning(f" 成本警告: 今日成本已达到 RMB {total_today:.4f}，超过阈值 RMB {threshold}",
...
def _get_project_config_dir():
    ...
    优先使用环境变量 STOCK_ANALYSIS_CONFIG_DIR 或 TRADINGAGENTS_CONFIG_DIR，
    便于 Stock_analysis 等独立项目使用自己的 config 目录，与 TradingAgents 严格分离。
    """
    config_dir = os.getenv("STOCK_ANALYSIS_CONFIG_DIR") or os.getenv("TRADINGAGENTS_CONFIG_DIR")
    if config_dir and config_dir.strip():
