...
from tradingagents.llm_adapters.llm_rate_control import attach_rate_control_to_llm
...
def create_fundamentals_analyst(llm, toolkit):
    ...
    def fundamentals_analyst_node(state):
        ...
        try:
            ...
            if tpl_info:
                logger.debug(f"[基本面分析师] 模板 source={tpl_info.get('source')}")
            ...
            logger.debug(f"[基本面分析师] 模板获取成功 长度={len(system_prompt)}")

        except Exception as e:
            logger.debug(f"[基本面分析师] 模板获取失败，使用硬编码: {e}")
            # 降级：使用硬编码提示词
            system_prompt = (
                f"你是一位专业的股票基本面分析师。"
            ...
            logger.debug(f"[基本面分析师] 使用降级提示词 长度={len(system_prompt)}")
        ...
        # Ensure any fresh llm instance still uses global rate control + 429 retry.
        provider_hint = "dashscope"
        model_hint = getattr(fresh_llm, "model_name", "unknown")
        class_name = fresh_llm.__class__.__name__.lower()
        ...
        fresh_llm = attach_rate_control_to_llm(
            fresh_llm,
        ...
        # 美股预处理：强制预先获取基本面数据，避免工具调用失败导致「数据获取失败」
        if market_info.get('is_us') and ticker:
            pre_fetched = None
            try:
                from tradingagents.dataflows.interface import get_fundamentals_openai
                logger.info(f"[基本面分析师] 预处理：强制获取 {ticker} 美股基本面数据...")
                pre_fetched = get_fundamentals_openai(ticker, current_date)
                if pre_fetched and "❌" not in pre_fetched and len(pre_fetched.strip()) > 200:
                    logger.info(f"[基本面分析师] 预处理成功，数据长度: {len(pre_fetched)}")
                else:
                    _len = len(pre_fetched.strip()) if pre_fetched else 0
                    logger.warning(f"[基本面分析师] 预处理数据无效或过短: {_len} 字符，尝试 Finnhub 兜底")
                    pre_fetched = None
            except Exception as e:
                logger.warning(f"[基本面分析师] 预处理失败: {e}，尝试 Finnhub 兜底")
                pre_fetched = None

            # 兜底：当 get_fundamentals_openai 失败时，直接调用 Finnhub（解决流程中取不到数据）
            if not pre_fetched and __import__("os").getenv("FINNHUB_API_KEY"):
                try:
                    from tradingagents.dataflows.interface import get_fundamentals_finnhub
                    logger.info(f"[基本面分析师] Finnhub 兜底：直接获取 {ticker}...")
                    pre_fetched = get_fundamentals_finnhub(ticker, current_date)
                    if pre_fetched and "❌" not in pre_fetched and len(pre_fetched.strip()) > 200:
                        logger.info(f"[基本面分析师] Finnhub 兜底成功，数据长度: {len(pre_fetched)}")
                    else:
                        pre_fetched = None
                except Exception as e2:
                    logger.warning(f"[基本面分析师] Finnhub 兜底失败: {e2}")

            if pre_fetched:
                tool_msg = ToolMessage(
                    content=pre_fetched,
                    tool_call_id="prefetch_fundamentals",
                    name="get_stock_fundamentals_unified"
                )
                state["messages"] = list(state["messages"]) + [tool_msg]
                logger.info(f"[基本面分析师] 已注入预处理数据，消息数: {len(state['messages'])}")
