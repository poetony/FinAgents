...
"""
    ...
    def get_realtime_stock_news(
        ...
        return get_realtime_stock_news(ticker, curr_date, hours_back=48)
...
"""
    ...
    def get_stock_news_unified(
        ...
        Returns:
            str: 新闻分析报告
        """
        logger.info(f"📰 [统一新闻工具] 分析股票: {ticker}，日期: {curr_date}")
        try:
            from tradingagents.dataflows.realtime_news_utils import get_realtime_stock_news
            # 与新闻分析师使用相同数据源（东方财富、FinnHub API、Alpha Vantage），确保流程中能取到数据
            result = get_realtime_stock_news(ticker, curr_date, hours_back=168)  # 7天
            if result and len(result.strip()) > 50:
                logger.info(f"📰 [统一新闻工具] 数据获取完成，长度: {len(result)}")
                return result
            return f"未获取到{ticker}的新闻数据，请检查股票代码或稍后重试。"
        except Exception as e:
